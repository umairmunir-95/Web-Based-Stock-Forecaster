...
session_start();
$sql2 = "SELECT max(indexValue),min(indexValue) FROM marketwatch";
$retval2 = mysql_query($sql2);
$i=0;
while($row2 = mysql_fetch_array($retval2, MYSQL_ASSOC))
{
	$_SESSION['maxIndex']=number_format(($row2["max(indexValue)"]),3);
	$_SESSION['minIndex']=number_format(($row2["min(indexValue)"]),3);
	$i++;
}

$sql3 = "select indexValue from marketwatch order by id desc limit 2";
...
while($row3 = mysql_fetch_array($retval3, MYSQL_ASSOC))
{
	if($i==0)
	{
		$num1=$row3["indexValue"];
	}
	else
	{
		$num2=$row3["indexValue"];
	}
	$i++;
}
$num=$num2-$num1;
$_SESSION['changedIndex']=number_format(($num),3);
...
_________________________________________________________ -->
 ...
 _________________________________________________________ -->
                    ...
                    <div class="col-md-9">
					<marquee width="800" scrollamount="10" scrolldelay="150"><?php 
					$myQuery = "SELECT ticker,Price FROM companies";
					$fetchedValue = mysql_query($myQuery);
					$i=0;
					while($row2 = mysql_fetch_array($fetchedValue, MYSQL_ASSOC))
					{
						echo '<strong>'.'<font color="red">'.$row2["ticker"].' : '.number_format(($row2["Price"]),2).' | '.'</font>'.'</strong>';
						$i++;
					}
					?></marquee>

					<center>
						<h2>Autonomic Stock Exchange Market Watch</h2>
					</center>
					...
					<div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                      <?php
									  $id="";
									  $q = "SELECT max(id) from marketwatch";
									  $ret = mysql_query($q);
									  while($row1 = mysql_fetch_array($ret, MYSQL_ASSOC))
									  {
										$id=$row1["max(id)"];
									  }
									  
									  $sql2 = "SELECT indexValue,volume,marketCapitalization FROM marketwatch where id=".$id;
									  $retval2 = mysql_query($sql2);
									  $i=0;
									  while($row2 = mysql_fetch_array($retval2, MYSQL_ASSOC))
									  {
									  ?>
										<td>
											<select name="type" id="type">
												<option value="Price | Change">ASE Index</option>
											</select>
										</td>
										<td><?php echo '<strong>'.'<font color="brown">'.number_format(($row2["indexValue"]),3).'</font>'.'</strong>'; ?></td>
										<td><?php if(($_SESSION['changedIndex'])<0) echo '<strong>'.'<font color="red">'.number_format(($_SESSION['changedIndex']),3).'</font>'.'</strong>'; else echo '<strong>'.'<font color="green">'.number_format(($_SESSION['changedIndex']),3).'</font>'.'</strong>'; ?></td>
										<td><?php echo  '<strong>'.'<font color="green">'.$_SESSION['maxIndex'].'</font>'.'</strong>';?></td>
										<td><?php echo  '<strong>'.'<font color="red">'.$_SESSION['minIndex'].'</font>'.'</strong>';?></td>
										<td><?php echo  '<strong>'. number_format(($row2["volume"]),3).'</strong>'; ?></td>
										<td><?php echo  '<strong>'.number_format(($row2["marketCapitalization"]),3).'</strong>'; ?></td>
										<td><a href="marketWatchGraph.php" target="_blank" class="link"><strong>View</strong></a></td> 
										</tr>
										<?php
										$i++;
										}
								 ...
								 <table class="table table-hover">
                                    ...
                                    <tbody>
									  ...
									  {
									  ?>
										<td><?php echo '<strong>'.'<font color="blue">'.$row2["ticker"].'</font>'.'</strong>'; ?></td>
										<td><?php echo '<strong>'.'<font color="brown">'.$row2["Name"].'</font>'.'</strong>'; ?></td>
										<td><?php echo  '<strong>'.'<font color="green">'.$row2["Stock"].'</font>'.'</strong>';?></td>
										<td><?php echo  '<strong>'.'<font color="green">'.number_format(($row2["Price"]),2).'</font>'.'</strong>';?></td>
										<td><?php if($row2["changedPrice"] < 0 ) echo  '<strong>'.'<font color="red">'.number_format(($row2["changedPrice"]),2).'</font>'.'</strong>'; 
										else echo  '<strong>'.'<font color="purple">'.number_format(($row2["changedPrice"]),2).'</font>'.'</strong>';
										?></td>
										<td><?php echo  '<strong>'.'<font color="brown">'.number_format(($row2["percentageChange"]),2).'</font>'.'</strong>';?></td>
										<td><a href="marketSummary.php" class="link"><strong>View</strong></a></td> 
										</tr>
										<?php
										$i++;
										}
