...
while($row2 = mysql_fetch_array($result2))
{
	$ticker=$row2["ticker"];
	$name=$row2["Name"];
	$currentPrice=number_format(($row2["Price"]+$row2["changedPrice"]),2); 
	$changedPrice=number_format(($row2["changedPrice"]),2);
	$percentageChange=number_format(($row2["percentageChange"]),2); 
	$volume=number_format(($row2["Stock"]),2);
	$i++;
}
$_Session['ti']=$ticker;
$_Session['na']=$name;
$_Session['crp']=$currentPrice;
$_Session['cnp']=$changedPrice;
$_Session['pc']=$percentageChange;
$_Session['v']=$volume;
                    ...
                    <div class="row portfolio">
						...
						<?php
							...
							while($row1 = mysql_fetch_array($result1))
							{
								...
								echo '<a href="modal.php?id='.$row1["ID"].'" class="btn btn-template-transparent-primary">View Details</a>';
								echo '</p>';
								echo '</div>';
								echo '</div>';
								echo '</div>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><?php echo $_Session['na'];?></h4>
            </div>
            <div class="modal-body">
				<form action="" method="post">
                   <div class="form-group">
				   <h5>Previous Price</h5>
				   <input type="text" class="form-control" id="email_modal" value="<?php echo $_Session['crp'];?>" readonly>
					</div>
					<h5>Changed Price</h5>
					<div class="form-group">
					<input type="text" class="form-control" id="password_modal" value="<?php echo $_Session['cnp'];?>" readonly>
					</div>
					<h5>%Change</h5>
					<div class="form-group">
					<input type="text" class="form-control" id="email_modal" value="<?php echo $_Session['pc'];?> " readonly>
					</div>
					<h5>Volume</h5>
					<div class="form-group">
					<input type="text" class="form-control" id="password_modal" value="<?php echo $_Session['v'];?>" readonly>
					</div>
					<p class="text-center">
					<?php
						echo '<a href="tradingForm.php?companyticker='.$_Session['ti'].'" class="btn btn-template-main">Click here to proceed!</a>';
					?>
					</p>		
                </form>
            </div>
